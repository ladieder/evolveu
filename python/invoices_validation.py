...
for row in prod_ws.iter_rows(min_row=2, max_col=4, values_only=True):
    # if prod_id is duplicate, don't add to prod_dict and give error message
    if row[0] in prod_dict:
...
for row in inv_ws.iter_rows(min_row=2, max_col=3, values_only=True):
    # if inv_id is duplicate, don't add and give error message
    if row[0] in inv_dict:
...
for row in lines_ws.iter_rows(min_row=2, max_col=3, values_only=True):
    # if a line item already exists in inv_dict for specified inv_id, set temp line_list to whatever already exists in inv_dict
    if "line_items" in inv_dict[row[0]]:
