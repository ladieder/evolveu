...
let processPeople = function(data, callback) {
	const filteredPeople = data.filter(function(data) {
		return (data.province === "AB" || data.province === "BC");
	});

	const mappedPeople = filteredPeople.map(callback);
};
//
// Here is invoking the processPeople function.  Write the callback
// function as an anonymous function
//
processPeople(people, function(person) {
	if (person.age > 25) {
...
let determineTotal = function() {
	...
	console.log("Number of people: ", count, ", Total age: ", total, ", Average age: ", Math.round(total/count));
};
